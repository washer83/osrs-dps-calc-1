...
export default class Comparator {
  ...
  constructor(
    ...
    this.xAxis = xAxis;
    this.yAxis = yAxis;
    this.commonOpts = { disableMonsterScaling: true };
  }

  private* inputsIterator(): Generator<InputSet> {
    const playerInput = (x: number, alterations: PartialDeep<Player>): InputSet =>
      ({ xValue: x, loadouts: this.baseLoadouts.map((p) => typedMerge(p, alterations)), monster: this.baseMonster });
    ...
    switch (this.xAxis) {
      ...
      case CompareXAxis.MONSTER_HP: {
        for (let newHp = this.baseMonster.skills.hp; newHp >= 0; newHp--) {
          yield { xValue: newHp, loadouts: this.baseLoadouts, monster: scaleMonsterHpOnly(merge(this.baseMonster, { inputs: { monsterCurrentHp: newHp } })) };
        }
        return;
      }
      ...
      case CompareXAxis.STAT_DECAY_RESTORE: {
        ...
        for (let restore = 0; restore <= limit; restore++) {
          ...
          yield { xValue: restore, loadouts: restoredLoadouts, monster: this.baseMonster };
        }
        return;
      }
      default:
